...
const ApiRequestLogsList: React.FC = () => {
  const [rows, setRows] = useState<UiApiRequestLog[]>([]);
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const list = await apiRequestLogsService.list({ limit: 100, offset: 0 });
      setRows(list);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to load API request logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((r) => `${r.method} ${r.endpoint}`.toLowerCase().includes(q));
  }, [rows, searchTerm]);

  const onDelete = async (id: string) => {
    if (!window.confirm('Delete this API request log?')) return;
    try {
      setBusy(true);
      setError(null);
      await apiRequestLogsService.remove(id);
      setRows((prev) => prev.filter((r) => r.id !== id));
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to delete API request log');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">Request logs (read-only)</p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {filtered.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-600">No API request logs found</p>
            </div>
          ) : (
            <ResponsiveTable
              data={filtered}
              ...
              columns={[
                {
                  ...
                  header: 'method',
                  render: (row) => (
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${methodBadgeClass(row.method)}`}>
                ...
                },
                {
                  ...
                  render: (row) => <span className="text-sm text-gray-700">{row.response_time || '—'}</span>,
                },
                {
                  key: 'created_at',
                  header: 'created_at',
      ...
      <RouteDetailsModal
        ...
        routePath={selectedId ? `/analytics/api-request-logs/${encodeURIComponent(selectedId)}` : null}
        headerTitle="API Request Log Details"
...
export default ApiRequestLogsList;
