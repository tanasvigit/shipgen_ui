...
import { AlertCircle, CalendarCheck, Edit, Search, Trash2 } from 'lucide-react';
import { scheduleAvailabilityService, type UiScheduleAvailability } from '../../services/scheduleAvailabilityService';
import { useToast } from '../ui/ToastProvider';
import Modal from '../common/Modal';
import ScheduleAvailabilityForm from './forms/ScheduleAvailabilityForm';

const ScheduleAvailabilityList: React.FC = () => {
  const { showToast } = useToast();
  const [rows, setRows] = useState<UiScheduleAvailability[]>([]);
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const list = await scheduleAvailabilityService.list({ limit: 100, offset: 0 });
      setRows(list);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to load availability records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      setError(null);
      await scheduleAvailabilityService.remove(id);
      setRows((prev) => prev.filter((r) => r.id !== id));
      showToast('Schedule availability deleted', 'success');
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to delete availability record');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Basic availability records</p>
        </div>
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Create Schedule Availability">
        <ScheduleAvailabilityForm mode="create" onCancel={() => setIsCreateOpen(false)} onSuccess={async () => {
          setIsCreateOpen(false);
          await load();
        }} />
      </Modal>
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Schedule Availability">
        <ScheduleAvailabilityForm mode="edit" availabilityId={selectedId ?? undefined} onCancel={() => setIsEditOpen(false)} onSuccess={async () => {
          setIsEditOpen(false);
          setSelectedId(null);
          await load();
        }} />
