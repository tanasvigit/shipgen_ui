...
import { AlertCircle, CalendarDays, Edit, Search, Trash2 } from 'lucide-react';
import { schedulesService, type UiSchedule } from '../../services/schedulesService';
...
const SchedulesList: React.FC = () => {
  ...
  const [rows, setRows] = useState<UiSchedule[]>([]);
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const list = await schedulesService.list({ limit: 100, offset: 0 });
      setRows(list);
    } catch (err: unknown) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Simple schedule records</p>
        </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((row) => (
                    <tr key={row.id} className="table-row table-row-zebra">
                      <td className="py-3 px-4 text-sm font-medium text-gray-900 table-cell-ellipsis" title={row.name || '—'}>{row.name || '—'}</td>
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Create Schedule">
        <ScheduleForm mode="create" onCancel={() => setIsCreateOpen(false)} onSuccess={async () => {
          setIsCreateOpen(false);
          await load();
        }} />
      </Modal>
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Schedule">
        <ScheduleForm mode="edit" scheduleId={selectedId ?? undefined} onCancel={() => setIsEditOpen(false)} onSuccess={async () => {
          setIsEditOpen(false);
          setSelectedId(null);
          await load();
        }} />
