...
router = APIRouter(prefix="/int/v1/catalog-categories", tags=["int-storefront-catalog-categories"])


class CatalogCategoryCreate(BaseModel):
    owner_uuid: str
    owner_type: str = "StorefrontCatalog"
    name: str
    description: Optional[str] = None
    for_type: str = "storefront_catalog"
...
def list_catalog_categories(
    ...
    categories = db.query(Category).filter(
        ...
        Category.for_type == "storefront_catalog",
...
def get_catalog_category(
    ...
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Catalog category not found")
...
def create_catalog_category(
    ...
    category.owner_uuid = payload.owner_uuid
    category.owner_type = payload.owner_type
